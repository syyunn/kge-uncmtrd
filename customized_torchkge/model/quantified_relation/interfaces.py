...
from torchkge.utils.dissimilarities import (
    l1_dissimilarity,
    l2_dissimilarity,
...
class Model(Module):
    ...
    def forward(self, heads, tails, negative_heads, negative_tails, relations, magnitudes):
        """
        ...
        """
        pos = self.scoring_function(heads, tails, relations, magnitudes)
        if negative_heads.shape[0] > relations.shape[0]:
            ...
            neg = self.scoring_function(
                negative_heads, negative_tails, relations.repeat(n_neg), magnitudes
            )
        else:
            neg = self.scoring_function(negative_heads, negative_tails, relations, magnitudes)
    ...
    def lp_compute_ranks(
        self, e_emb, candidates, r, e_idx, r_idx, true_idx, dictionary, m, heads=1
    ):
        ...
        if heads == 1:
            scores = self.lp_scoring_function(e_emb, candidates, r, m)
        else:
            scores = self.lp_scoring_function(candidates, e_emb, r, m)
    ...
    def lp_helper(self, h_idx, t_idx, r_idx, kg, m):
        ...
        rank_true_tails, filt_rank_true_tails = self.lp_compute_ranks(
            h_emb, candidates, r, h_idx, r_idx, t_idx, kg.dict_of_tails, m, heads=1
        )
        rank_true_heads, filt_rank_true_heads = self.lp_compute_ranks(
            t_emb, candidates, r, t_idx, r_idx, h_idx, kg.dict_of_heads, m, heads=-1
        )
...
class TranslationModel(Model):
    ...
    def lp_scoring_function(self, proj_h, proj_t, r, m):
        ...
        # r_norm_dev = torch.norm(r, p=2, dim=1)

        r_normalized = normalize(r, p=2, dim=1)

        # r_normalized_norm_dev = torch.sum(r_normalized, p=2, dim=1)

        magnitude_expanded = m.reshape([b_size, 1]).expand(b_size, self.emb_dim)
        ...
        if len(proj_h.shape) == 2 and len(proj_t.shape) == 3:
            ...
            hr = (proj_h + r_scaled).view(b_size, 1, r.shape[1])
            ...
            r_scaled_ = r_scaled.view(b_size, 1, r_scaled.shape[1])
            t_ = proj_t.view(b_size, 1, r_scaled.shape[1])
            return -self.dissimilarity(proj_h + r_scaled_, t_)

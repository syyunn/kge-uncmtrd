...
from customized_torchkge.data_loader_eval import DataLoader
    ...
    * Antoine Bordes, Nicolas Usunier, Alberto Garcia-Duran, Jason Weston,
    ...
    def evaluate(self, b_size, k_max, verbose=True):
        ...
        for i, batch in tqdm(
            ...
            h_idx, t_idx, r_idx, m = batch[0], batch[1], batch[2], batch[3]

            rk_true_t, f_rk_true_t, rk_true_h, f_rk_true_h = self.model.lp_helper(
                h_idx, t_idx, r_idx, self.kg, m
            )

            self.rank_true_heads[i * b_size : (i + 1) * b_size] = rk_true_h
            self.rank_true_tails[i * b_size : (i + 1) * b_size] = rk_true_t

            self.filt_rank_true_heads[i * b_size : (i + 1) * b_size] = f_rk_true_h
            self.filt_rank_true_tails[i * b_size : (i + 1) * b_size] = f_rk_true_t
    ...
    def mean_rank(self):
        ...
        if not self.evaluated:
            raise NotYetEvaluatedError(
                "Evaluator not evaluated call " "LinkPredictionEvaluator.evaluate"
            )
        sum_ = (
            self.rank_true_heads.float().mean() + self.rank_true_tails.float().mean()
        ).item()
        filt_sum = (
            self.filt_rank_true_heads.float().mean()
            + self.filt_rank_true_tails.float().mean()
        ).item()
    ...
    def mrr(self):
        ...
        return (
            (head_mrr + tail_mrr).item() / 2,
            (filt_head_mrr + filt_tail_mrr).item() / 2,
        )

    def print_results(self, model_prefix, epoch, k=None, n_digits=3):
        """
        ...
        """
        f = open(os.path.join(model_prefix, "eval_log.txt"), "a+")
        f.write("======================================\n")
        ...
        if k is not None and type(k) == int:
            print(
                "Hit@{} : {} \t\t Filt. Hit@{} : {}".format(
                    k,
                    round(self.hit_at_k(k=k)[0], n_digits),
                    k,
                    round(self.hit_at_k(k=k)[1], n_digits),
                )
            )
        ...
        if k is not None and type(k) == list:
            for i in k:
                print(
                    "Hit@{} : {} \t\t Filt. Hit@{} : {}".format(
                        i,
                        round(self.hit_at_k(k=i)[0], n_digits),
                        i,
                        round(self.hit_at_k(k=i)[1], n_digits),
                    )
                )
        ...
        print(
            "Mean Rank : {} \t Filt. Mean Rank : {}".format(
                int(self.mean_rank()[0]), int(self.mean_rank()[1])
            )
        )

        f.write(
            "Mean Rank : {} \t Filt. Mean Rank : {}\n".format(
                int(self.mean_rank()[0]), int(self.mean_rank()[1])
            )
        )
        print(

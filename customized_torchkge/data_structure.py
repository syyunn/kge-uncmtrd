"""
Customize KnowledGraph Dataset To Encompass the Relation's Magnitude
"""

from collections import defaultdict
from torch import cat, eq, int64, long, randperm, tensor, Tensor, zeros_like, float64
...
class KnowledgeGraph(Dataset):
    ...
    def __getitem__(self, item):
        return (
            self.head_idx[item].item(),
            self.tail_idx[item].item(),
            self.relations[item].item(),
            self.magnitude[item].item()
        )
    ...
    def sanity_check(self):
        ...
        assert (
            (type(self.head_idx) == Tensor)
            & (type(self.tail_idx) == Tensor)
            & (type(self.relations) == Tensor)
        )
        assert (
            (self.head_idx.dtype == int64)
            & (self.tail_idx.dtype == int64)
            & (self.relations.dtype == int64)
        )
    ...
    def split_kg(self, share=0.8, sizes=None, validation=False):
        ...
        if sizes is not None:
            try:
                ...
                raise SizeMismatchError(
                    "Tuple `sizes` should sum up to the "
            ...
            return (
                KnowledgeGraph(
                    kg={
                        ...
                        "magnitudes": self.magnitudes[mask_tr]
                    },
                    ent2ix=self.ent2ix,
                    rel2ix=self.rel2ix,
                ...
                KnowledgeGraph(
                    kg={
                        ...
                        "magnitudes": self.magnitudes[mask_val]
                    },
                    ent2ix=self.ent2ix,
                    rel2ix=self.rel2ix,
                    dict_of_heads=self.dict_of_heads,
                ...
                KnowledgeGraph(
                    kg={
                        ...
                        "magnitudes": self.magnitudes[mask_te]
                    },
                    ent2ix=self.ent2ix,
                    rel2ix=self.rel2ix,
                    dict_of_heads=self.dict_of_heads,
    ...
    def get_mask(self, share, validation=False):
        ...
        for i, r in enumerate(uniques_r):
            ...
            if validation:
                train_size, val_size, test_size = self.get_sizes(
                    counts_r[i], share=share, validation=True
                )
                ...
                train_size, test_size = self.get_sizes(
                    counts_r[i], share=share, validation=False
                )
                mask[sub_mask[rand[:train_size]]] = True
        ...
        if len(u) < self.n_ent:
            missing_entities = tensor(
                list(set(uniques_e.tolist()) - set(u.tolist())), dtype=long
            )
            for e in missing_entities:
                sub_mask = ((self.head_idx == e) | (self.tail_idx == e)).nonzero()[:, 0]
                rand = randperm(len(sub_mask))
                sizes = self.get_sizes(
                    mask.shape[0], share=share, validation=validation
                )
    ...
    def evaluate_dicts(self):
        ...
        for i in range(self.n_facts):
            self.dict_of_heads[(self.tail_idx[i].item(), self.relations[i].item())].add(
                self.head_idx[i].item()
            )
            self.dict_of_tails[(self.head_idx[i].item(), self.relations[i].item())].add(

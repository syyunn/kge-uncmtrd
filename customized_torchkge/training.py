...
from customized_torchkge.evaluation import LinkPredictionEvaluator
import os
import pickle
import torch
...
class TrainDataLoaderIter:
    def __init__(self, loader):
        ...
        self.nh, self.nt = loader.sampler.corrupt_kg(loader.b_size, loader.tmp_cuda)
        if loader.use_cuda:
    ...
    def __next__(self):
        if self.current_batch == self.n_batches:
            ...
            batch["m"] = self.m[i * self.b_size : (i + 1) * self.b_size]
...
class Trainer:
    ...
    def run(self, kg_test, model_prefix):
        ...
        for epoch in iterator:
            ...
            with open(os.path.join(model_prefix, "emb_idx.pickle"), "wb") as handle:
                pickle.dump(
                    {"ent2ix": self.kg_train.ent2ix, "rel2ix": self.kg_train.rel2ix},
                    handle,
            ...
            if epoch % 10 == 0:
                ...
                evaluator.print_results(model_prefix, epoch)
                torch.save(
                    self.model.state_dict(), os.path.join(model_prefix, "model.torch")
                )
